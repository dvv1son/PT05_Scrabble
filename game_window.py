...
BONUS_VIEW = {
    "normal": {"text": "", "bg": "#efe6d6", "fg": "#2f2f2f"},
    "double_letter": {"text": "2Б", "bg": "#8fcaf0", "fg": "#12344d"},
    "triple_letter": {"text": "3Б", "bg": "#4c8bea", "fg": "white"},
    "double_word": {"text": "2С", "bg": "#e9a8bd", "fg": "#5e1b2c"},
    "triple_word": {"text": "3С", "bg": "#ee6666", "fg": "white"},
    "start": {"text": "★", "bg": "#f2c744", "fg": "#4f3d00"},
}
...
class GameWindow(tk.Frame):
    def __init__(self, master, app, words, letters_scores, board_layout):
        ...
        self.players = [
            {"name": "Игрок 1", "score": 0, "rack": []},
            {"name": "Игрок 2", "score": 0, "rack": []},
        ]
    ...
    def start_game(self):
        ...
        self.players[0]["rack"] = self.create_rack_from_word(start_word) if start_word else []
        self.refill_rack(0)
        self.refill_rack(1)
    ...
    def create_widgets(self):
        tk.Label(
            self, text="Скрэббл", font=("Segoe UI", 18, "bold"),
            bg="#f7f7f3", fg="#063f25"
        ).pack(pady=(8, 4))
        ...
        left_part.pack(side="left", padx=(0, 12))

        right_part = tk.Frame(content, bg="#ffffff", width=250, height=460, bd=1, relief="groove")
    ...
    def create_board_canvas(self, parent):
        self.board_canvas = tk.Canvas(
            parent, width=BOARD_CANVAS_SIZE, height=BOARD_CANVAS_SIZE,
            bg="#f7f7f3", highlightthickness=0
        )
    ...
    def create_side_panel(self, side):
        ...
        tk.Label(
            panel_inner, textvariable=self.current_player_var,
            font=("Segoe UI", 13, "bold"), bg="#ffffff",
            wraplength=220, justify="left"
        ).pack(anchor="w", pady=(0, 16))

        tk.Label(panel_inner, text="Счёт", font=("Segoe UI", 12, "bold"), bg="#ffffff").pack(anchor="w")

        tk.Label(
            panel_inner, textvariable=self.score_var,
            font=("Segoe UI", 11), bg="#ffffff",
            justify="left", wraplength=220
        ).pack(anchor="w", pady=(4, 16))

        tk.Label(
            panel_inner, text="Выбранная буква",
            font=("Segoe UI", 12, "bold"), bg="#ffffff"
        ).pack(anchor="w")

        tk.Label(
            panel_inner, textvariable=self.selected_var,
            font=("Segoe UI", 11), bg="#ffffff",
            justify="left", wraplength=220
        ).pack(anchor="w", pady=(4, 16))

        tk.Label(
            panel_inner, textvariable=self.bag_var,
        ...
        button_style = {
            ...
            "bg": "#f8f8f8", "activebackground": "#e7f2ec",
            "wraplength": 210, "justify": "center"
        }

        tk.Button(panel_inner, text="Проверить и подтвердить ход", command=self.confirm_move, **button_style).pack(fill="x", pady=4)
        tk.Button(panel_inner, text="Сбросить размещение", command=self.cancel_pending_move, **button_style).pack(fill="x", pady=4)
        tk.Button(panel_inner, text="Обновить буквы", command=self.exchange_letters, **button_style).pack(fill="x", pady=4)
        tk.Button(panel_inner, text="Пропустить ход", command=self.skip_turn, **button_style).pack(fill="x", pady=4)
        tk.Button(panel_inner, text="Завершить игру", command=self.finish_game, **button_style).pack(fill="x", pady=(20, 4))
        tk.Button(panel_inner, text="В главное меню", command=self.app.show_menu, **button_style).pack(fill="x", pady=4)
    ...
    def create_rack_panel(self):
        ...
        tk.Label(
            rack_frame, text="Буквы текущего игрока:",
            font=("Segoe UI", 11, "bold"), bg="#f7f7f3"
        ...
        for i in range(7):
            btn = tk.Button(
                rack_frame, width=5, height=2, font=("Segoe UI", 12, "bold"),
                bg="#fff2d9", activebackground="#f7dfb8",
    ...
    def get_cell_view(self, cell):
        if cell.letter:
            ...
            bg = "#cfeecd" if cell.is_preview else "#fff2d9"
            return {"text": f"{cell.letter}\n{score}", "bg": bg, "fg": "#1e1e1e"}
    ...
    def draw_headers(self):
        for col in range(BOARD_SIZE):
            ...
            self.board_canvas.create_text(
                x, HEADER_SIZE / 2, text=str(col + 1),
                font=("Segoe UI", 8, "bold"), fill="#1f1f1f"
            )
        ...
        for row in range(BOARD_SIZE):
            for col in range(BOARD_SIZE):
                ...
                self.board_canvas.create_rectangle(
                    x1, y1, x2, y2, fill=view["bg"],
                    outline="#a99f91", width=1
                )

                if view["text"]:
                    font = ("Segoe UI", 10, "bold") if cell.letter else ("Segoe UI", 8, "bold")
                    self.board_canvas.create_text(
                        (x1 + x2) / 2, (y1 + y2) / 2,
                        text=view["text"], font=font, fill=view["fg"], justify="center"
                    )

    def on_canvas_click(self, event):
        if event.x < HEADER_SIZE or event.y < HEADER_SIZE:
            return

        col = int((event.x - HEADER_SIZE) // CELL_SIZE)
        row = int((event.y - HEADER_SIZE) // CELL_SIZE)
    ...
    def refresh_rack(self):
        ...
        for i, btn in enumerate(self.rack_buttons):
            ...
            bg = "#cfeecd" if self.selected_rack_index == i else "#fff2d9"
            relief = "sunken" if self.selected_rack_index == i else "raised"
            btn.config(bg=bg, relief=relief)

    def refresh_info(self):
        player = self.players[self.current_player]
        self.current_player_var.set(f"Ходит: {player['name']}")
        ...
        letter = player["rack"][self.selected_rack_index]
        self.selected_var.set(f"{letter} ({self.letters_scores.get(letter, 0)} очк.)" if letter else "не выбрана")
    ...
    def on_rack_click(self, rack_index):
        ...
        self.selected_rack_index = None if self.selected_rack_index == rack_index else rack_index
        self.refresh_rack()
        self.refresh_info()
    ...
    def on_board_click(self, row, col):
        ...
        if self.selected_rack_index is None:
            messagebox.showinfo("Скрэббл", "Сначала выберите букву на подставке.")
            return

        if not cell.is_empty:
            messagebox.showwarning("Скрэббл", "Эта клетка уже занята.")
            return

        rack = self.players[self.current_player]["rack"]
        letter = rack[self.selected_rack_index]

        self.pending_tiles[(row, col)] = {"letter": letter, "rack_index": self.selected_rack_index}
    ...
    def confirm_move(self):
        ok, error_text, words, score = evaluate_move(
            self.board, self.pending_tiles.keys(), self.words, self.letters_scores
        )
        ...
        commit_move(self.board, self.pending_tiles.keys(), self.current_player + 1)
    ...
    def exchange_letters(self):
        if self.pending_tiles:
            messagebox.showwarning("Скрэббл", "Сначала сбросьте размещённые буквы на поле.")
            return

        if not messagebox.askyesno("Обновить буквы", "Заменить все буквы текущего игрока и передать ход?"):
            return
    ...
    def finish_game(self):
        ...
        result = {"score_1": p1, "score_2": p2, "winner": winner}

        EndWindow(
            self, result=result, on_new_game=self.app.start_new_game,
            on_back_to_menu=self.app.show_menu, on_exit=self.app.root.destroy
        )